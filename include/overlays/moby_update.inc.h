...
extern short D_8006CBEC[3][2];

extern int D_8006E52C[];
extern Vector3D16 D_8006E614[3][2];
...
extern ByteXY D_8006E638[32];

extern SphericalCoordsOffset D_8006CA24[];

extern short D_80073094[];

#define MOBY_COLOR32(m) (*(int *)&(m)->m_SpecularMetalColor[0])
...
void NAME_OVERLAY_FUNCTION(UpdateMoby)(void) {
  ...
  while (moby = *mobyList++) {
    ...
    // Seems to be a macro
    D_80075794 =
        moby->m_AnimationState.m_AnimationFlags & 2; // iirc, animation finished
    D_800757F4 =
        moby->m_AnimationState.m_AnimationFlags & 1; // iirc, frame finished
    D_800756C4 = localDeltaTime; // Deltatime used in overlays, dunno why

    switch (mobyClass) {
#ifdef HAS_PORTAL
    case MOBYCLASS_PORTAL: { // Portal text
      ...
      if (distance < (moby->m_State ? 12 : 11) * 1024) {
        ...
        cameraPortalAngleDiff =
            func_80017908(moby->m_Rotation.z,
                          Atan2(g_Camera.m_Position.x - moby->m_Position.x,
        ...
        if (cameraPortalAngleDiff <= ROTDEG(80)) {
          ...
          moby->m_Substate = 0;
                   ...
                   ROTDEG(100)) { // You're facing against the rotation of the
                                  // portal
      ...
      for (i = 0; i < g_DeltaTime; i++) {
        ...
        if (distance < 0x4000) {
          if ((vortexProps->m_ParticleTimer & 3) == 0) {
            D_800758E4(1, 0x15, &moby->m_Position,
                       (void *)vortexProps->m_ParticleTimer);
          }
        } else if ((vortexProps->m_ParticleTimer & 0xf) < 8) {
          if ((vortexProps->m_ParticleTimer & 3) == 0) {
            D_800758E4(1, 0x4c, &moby->m_Position,
                       (void *)vortexProps->m_ParticleTimer);
          }
        }
      }
    ...
    case MOBYCLASS_GEM_SPAWNER: {
      ...
      if (spawnerProps->m_InitDone == 0) {
        if (spawnerProps->m_RelativeMode != 0) {
          VecSub(&spawnerProps->m_Position, &spawnerProps->m_Position,
                 &parent->m_Position);
      ...
      if (spawnerProps->m_SpawnTimer != 0) {
        if (func_80037F90(&spawnerProps->m_SpawnTimer, 4) != 0) {
          if (spawnerProps->m_RelativeMode != 0) {
            VecAdd(&spawnerProps->m_Position, &spawnerProps->m_Position,
                   &moby->m_Position);
          } else {
            VecCopy(&moby->m_Position, &parent->m_Position);
          }
    ...
    case MOBYCLASS_BUTTERFLY: { // Butterfly
          ...
          if (g_Sparx == nullptr) {             // If Sparx is not alive
            ...
            if (sparxProps->m_MobyPickingUp == nullptr) {
              ...
              if (func_800381BC(
                      ...
                      ANGLE_TO(moby->m_Position, g_Spyro.m_Position)) < 0) {
      ...
      case 0: {
        ...
        if (func_80037F90(&butterflyProps->m_unk_14, 2) == 0 ||
            moby->m_WasDrawn) {
          ...
          if (func_80037F90(&butterflyProps->m_TurnRetargetTimer, 1) != 0) {
            ...
            butterflyProps->m_TargetAngle =
                func_80038074(butterflyProps->m_TargetAngle, r);
          ...
          if (butterflyProps->m_VerticalSpeed >= 128) {
            moby->m_Position.z =
                moby->m_Position.z + (butterflyProps->m_VerticalSpeed - 256);
          } else {
            moby->m_Position.z =
                moby->m_Position.z + butterflyProps->m_VerticalSpeed;
          }
          ...
          rotated =
              RotateMobyToAngle(moby, butterflyProps->m_TargetAngle, 4, 0xA, 1);
          moved = func_80039398(moby, 30, 0, 50, 1);

          if (rotated && moved) {
            butterflyProps->m_TargetAngle = func_80038074(
                butterflyProps->m_TargetAngle, RandRange(98, 158));
            butterflyProps->m_TurnRetargetTimer = RandRange(30, 80);
          }

          // Check distance to target position
          if (OctDistance(&moby->m_Position,
                          &butterflyProps->m_AnchorPosition) > 2000) {
      ...
      case 1: {
        func_80038638(
            moby, &g_Spyro.m_Position, 0x320,
            func_80038074(ANGLE_TO(moby->m_Position, g_Spyro.m_Position),
                          butterflyProps->m_SparxApproachSide * 8),
    ...
    case MOBYCLASS_DRAGON_EGG: { // Dragon Egg
      ...
      case 2: {
        ...
        if (eggProps->m_Timer < 128) {
          ...
          moby->m_Position.x +=
              FIXED_MUL(Cos(eggProps->m_Angle * 16), eggProps->m_SpiralRadius);
          moby->m_Position.y +=
              FIXED_MUL(Sin(eggProps->m_Angle * 16), eggProps->m_SpiralRadius);
      ...
      case 4: {
        ...
        if (eggProps->m_Timer < 128) {
          ...
          // Apply circular motion
          moby->m_Position.x +=
              (Cos(eggProps->m_Angle * 0x10) * eggProps->m_SpiralRadius) >> 12;
          moby->m_Position.y +=
              (Sin(eggProps->m_Angle * 0x10) * eggProps->m_SpiralRadius) >> 12;
...
#define HAS_ANY_CHEST_FRAGMENTS
    case MOBYCLASS_EXTRA_LIFE_FRAG_1:
    case MOBYCLASS_EXTRA_LIFE_FRAG_2:
#endif
#ifdef HAS_ANY_CHEST_FRAGMENTS
    {
      ...
      if (physicsProps->m_Lifetime > 0 && moby->m_WasDrawn) {
        if (func_8004BE4C(&moby->m_Position, 0x100, 0x100)) {
          ...
          dot = physicsProps->m_VelocityX * g_CollisionNormal.x +
...
#ifdef HAS_EXTRA_LIFE_CHEST
#define HAS_ANY_CHEST_FRAGMENTS
    case MOBYCLASS_EXTRA_LIFE_FRAG_3:
#endif
#ifdef HAS_ANY_CHEST_FRAGMENTS
    {
      MobyFragmentPhysicsProps *physicsProps = moby->m_Props;
      ...
      if (physicsProps->m_Lifetime != 0 && moby->m_WasDrawn &&
          moby->m_Position.z > physicsProps->m_KillBelowZ) {
...
#undef HAS_ANY_CHEST_FRAGMENTS
    ...
    case MOBYCLASS_GEM_25: {
      ...
      case 0: {
        ...
        collectableProps->m_RotX = -Atan2Fast(
      ...
      case 2: {
        ...
        VecMult(&collectableMovePos, &collectableProps->m_VelocityOrPickupPos,
                g_DeltaTime);
          ...
          if (func_8004BE4C(&collectableMovePos, 240, 240)) {
            ...
            if (collectableProps->m_BounceCount != 0) {
              ...
              g_Spu.m_VolumeOverride.left =
                  0x3CCC >> (3 - collectableProps->m_BounceCount);
              ...
              if (func_80017428(&collectableProps->m_VelocityOrPickupPos,
                                &collectableSurfaceNormal,
                                &collectableProps->m_VelocityOrPickupPos)) {
                collectableProps->m_BounceCount--;

                // Reduce velocity each bounce with randomness
                collectableProps->m_VelocityOrPickupPos.x =
                    (collectableProps->m_VelocityOrPickupPos.x >> 3) +
                    (rand() & 0x3F) - 32;
                collectableProps->m_VelocityOrPickupPos.y =
                    (collectableProps->m_VelocityOrPickupPos.y >> 3) +
                    (rand() & 0x3F) - 32;
                collectableProps->m_VelocityOrPickupPos.z =
                    (collectableProps->m_VelocityOrPickupPos.z >> 2) +
                    (rand() & 0xF);
              }
            }
        ...
        if (distanceToSpyro < 1434) {
          ...
          if (ABS2(zDist) < 512) {
            ...
            if (g_Sparx != nullptr) {
              ...
              if (g_Spyro.m_airTime == 0 &&
                  collectableProps->m_BounceCount < 3) {
      ...
      case 3: { // After being picked up, move towards spyro
        if (collectableProps->m_Ticks >= 32) {
          ...
          if (VecMagnitude(&collectablePickupDelta, 1) > 480) {
            ...
            VecAdd(&moby->m_Position, &collectableProps->m_VelocityOrPickupPos,
    ...
    case MOBYCLASS_DRAGON_PAD_FAIRY: { // Dragon Pad Fairy
      ...
      case 2: {
        if (fairyProps->m_Timer != 0) {
          ...
          if (DISTANCE_TO_SPYRO(moby) < 0x2000) {
            if (ABS2(moby->m_Position.z - moby->m_FloorDistance -
                     g_Spyro.m_Position.z) < 2048) {
      ...
      case 4: { // Idle hovering
        ...
        fairyProps->m_Timer++;
        moby->m_Position.z =
            fairyProps->m_BaseZ + (Cos(fairyProps->m_Timer << 7) >> 6);
        ...
        if ((padMoby->m_State == 0 ||
            ...
            ABS2(padMoby->m_Position.z - padMoby->m_FloorDistance -
                 g_Spyro.m_Position.z) < 512) {
        ...
        if (DISTANCE_TO_SPYRO(moby) > 0x2400 ||
            (ABS2(moby->m_Position.z - moby->m_FloorDistance -
          ...
          if (fairyProps->m_RetargetTimer == 0) {
            ...
            char finalAngle =
                (ANGLE_TO(g_Spyro.m_Position, padMoby->m_Position) +
                 (rand() & 0xF) - 8);
            ...
            fairyProps->m_TargetPosition.x -=
                (Cos(finalAngle * 0x10) * baseDistance) >> 12;
            fairyProps->m_TargetPosition.y -=
                (Sin(finalAngle * 0x10) * baseDistance) >> 12;

            fairyProps->m_TargetAngle =
                ANGLE_TO(g_Spyro.m_Position, fairyProps->m_TargetPosition);
      ...
      case 5: {
        ...
        if (DISTANCE_TO_SPYRO(moby) > 0x2800 ||
          ...
          angleDiff =
              func_80017948(fairyProps->m_TargetAngle, moby->m_Rotation.z);
    ...
    case MOBYCLASS_GNORC_WARRIOR: {
      ...
      case 0:
        ...
        if (DISTANCE_TO_SPYRO(moby) < 0x2000 &&
            ABS2(moby->m_Position.z - g_Spyro.m_Position.z) < 0x5dc) {
          props->m_Timer = 0x48;
          moby->m_State = 2;
        ...
        if (props->m_TargetMobyIndex != -1) {
          if (func_80037F90(&props->m_WaitTimer, 4) != 0) {
            Moby *targetMoby;
            int angle;
            targetMoby = &g_LevelMobys[props->m_TargetMobyIndex];
            angle = Atan2(targetMoby->m_Position.x - moby->m_Position.x,
                          targetMoby->m_Position.y - moby->m_Position.y, 0);
      ...
      case 7:
        if (D_80075794) {
          if (!func_80037F90(&props->m_AttackLatchOrTimer, 4)) {
            func_800529E4(moby, 1);
          } else {
            moby->m_State = 0;
            if (moby->m_AnimationState.m_NextAnimation != 0) {
      ...
      case 10:
        ...
        case 1:
          ...
          if (D_800757F4 && moby->m_AnimationState.m_Frame == 8) {
            ...
            g_LevelMobys[props->m_TargetMobyIndex]
                .m_AnimationState.m_Animation = 13;
            g_LevelMobys[props->m_TargetMobyIndex]
                .m_AnimationState.m_NextAnimation = 13;
            g_LevelMobys[props->m_TargetMobyIndex].m_AnimationState.m_Frame = 0;
            g_LevelMobys[props->m_TargetMobyIndex]
                .m_AnimationState.m_NextFrame = 0;
            g_LevelMobys[props->m_TargetMobyIndex]
                .m_AnimationState.m_FrameProgress = 0;
          }
          if (D_80075794) {
            moby->m_Substate++;
          }
    ...
    case MOBYCLASS_SPARX: {
      ...
      if (g_Spyro.m_health >= 2) {
        ...
        vec_88.y = -0x64;
        vec_88.z = 0;
        VecRotateByLastMatrix(&vec_88, &vec_88);
        VecAdd(&vec_88, &vec_88, &moby->m_Position);
        D_800758E4(2, 0x42, &vec_88, nullptr);
      }
      ...
      if (sparxProps->m_MobyPickingUp != nullptr) {
        ...
        case 0: { /* Approach the moby being picked up */
          ...
          if (isPickupAnimPhase == 0) {
            vec_38.x -=
                COSINE_8(sparxProps->m_MobyPickingUp->m_Rotation.z) >> 4;
            vec_38.y -= SINE_8(sparxProps->m_MobyPickingUp->m_Rotation.z) >> 4;
          } else {
            vec_38.x -=
                COSINE_8(sparxProps->m_MobyPickingUp->m_Rotation.z) >> 6;
          ...
          if (isPickupAnimPhase == 0) {
            if (func_80037F90(sparxProps, 4) != 0 ||
                (sparxProps->m_Timer < 80 &&
                 func_80017908(
                     (moby->m_Rotation.z + (bp->m_SparxApproachSide * 110)) &
                         0xFF,
                     ((g_Camera.m_Rotation.z >> 4) + 0x80) & 0xFF) < 8)) {
        ...
        case 4: { /* Chasing a gem to gather it */
          ...
          VecSub(&sparxChaseDelta, &sparxProps->m_MobyPickingUp->m_Position,
                 &moby->m_Position);
          ...
          if (dist < 512) {
            ...
            VecCopy(&cp->m_VelocityOrPickupPos,
                    &sparxProps->m_MobyPickingUp->m_Position);
            ...
            if (sparxProps->m_MobyPickingUp->m_Class == 14)
              ...
              D_800758E4(1, 0xC, moby,
                         (void *)*(&D_8006E330 +
                                   sparxProps->m_MobyPickingUp->m_Class));
            ...
            // i don't really get this, we already know dist >= 512 here...maybe
            // some sort of inline
            int dcheck = 310;
...
#if LEVEL == 12
    case MOBYCLASS_DARKHOLLOW_LAMP: {
      switch (moby->m_State) {
      case 0:
    ...
    case MOBYCLASS_DARKHOLLOW_FIRE: {
    ...
    case MOBYCLASS_GNORC_SOLDIER: {
      ...
      case 0:
        ...
        case 0:
          if (OctDistance(&g_Spyro.m_Position,
                          &props->m_Path->m_Nodes[0].m_Position) < 0x1c00) {
            if (func_80017908(Atan2(g_Spyro.m_Position.x - moby->m_Position.x,
                                    g_Spyro.m_Position.y - moby->m_Position.y,
                                    0),
                              Atan2(props->m_Path->m_Nodes[0].m_Position.x -
                                        moby->m_Position.x,
                                    props->m_Path->m_Nodes[0].m_Position.y -
                                        moby->m_Position.y,
                                    0)) >= 0x15 ||
                DISTANCE_TO_SPYRO(moby) >
                    OctDistance(&moby->m_Position,
                                &props->m_Path->m_Nodes[0].m_Position)) {
              props->m_Path->m_CurrentNode = 0;
              moby->m_State = 3;
              break;
            }
          } else if (OctDistance(&g_Spyro.m_Position,
                                 &props->m_Path->m_Nodes[1].m_Position) <
                     0x1c00) {
            if (func_80017908(Atan2(g_Spyro.m_Position.x - moby->m_Position.x,
                                    g_Spyro.m_Position.y - moby->m_Position.y,
                                    0),
                              Atan2(props->m_Path->m_Nodes[1].m_Position.x -
                                        moby->m_Position.x,
                                    props->m_Path->m_Nodes[1].m_Position.y -
                                        moby->m_Position.y,
                                    0)) >= 21 ||
                DISTANCE_TO_SPYRO(moby) >
                    OctDistance(&moby->m_Position,
        ...
        case 1:
          ...
          func_80039E94(moby, props->m_Path, 0x100, 0x5a, 0xc8, 5, 0xa,
                        moby->m_Pod, 5);
        ...
        case 3:
          ...
          case 0:
            ...
            if (DISTANCE_TO_SPYRO(moby) < 0x2000 &&
                moby->m_Position.z - moby->m_FloorDistance -
                        g_Spyro.m_Position.z <
                    0 &&
                g_Spyro.m_airTime == 0) {
              linkedProps =
                  (Moby165Props *)g_LevelMobys[props->m_LinkedMobyIndex]
                      .m_Props;
          ...
          case 1:
            if (func_80037F90(&props->m_Timer, 4) != 0) {
              props->m_AttackState++;
            }
          ...
          case 3:
            ...
            if (func_80039E94(moby, props->m_Path, 0x100, 0x96, 0, 8, 0x32,
                              0xff, 5) == 0x100) {
              props->m_AttackState++;
            }
      ...
      case 1:
        ...
        case 0:
          ...
          if (func_80037F90(&props->m_Timer, 4) != 0 && D_80075794) {
        ...
        case 1:
          if (func_80037F90(&props->m_Timer, 4) != 0 &&
              DISTANCE_TO_SPYRO(moby) < 0x578 &&
              ABS2(moby->m_Position.z - moby->m_FloorDistance -
                   g_Spyro.m_Position.z) < 0x3e8 &&
      ...
      case 2:
        ...
        MoveMobyWithGravity(moby, &props->m_KnockbackTimer,
                            props->m_KnockbackAngle, &props->m_KnockbackZVel,
                            0xc, 0xc);

        if (D_80075794 && moby->m_AnimationState.m_NextAnimation == 2 &&
      ...
      case 3:
        ...
        angle = Atan2(
            props->m_Path->m_Nodes[props->m_Path->m_CurrentNode].m_Position.x -
                moby->m_Position.x,
            props->m_Path->m_Nodes[props->m_Path->m_CurrentNode].m_Position.y -
                moby->m_Position.y,
            0);
        ...
        if (OctDistance(&moby->m_Position,
                        &props->m_Path->m_Nodes[props->m_Path->m_CurrentNode]
                             .m_Position) < 0x100 ||
    ...
    case MOBYCLASS_GNORC_SENTRY: {
      ...
      case 0:
        ...
        if (func_80017908(pathAngle, moby->m_Rotation.z) < 0x10) {
          ...
          if (props->m_CloseEnough != 0 ||
              OctDistance(&moby->m_Position,
                          &props->m_Path->m_Nodes[props->m_Path->m_CurrentNode]
                               .m_Position) < 0x80) {
            if (props->m_Path->m_Reversed == 1) {
              if (props->m_Path->m_CurrentNode + 1 ==
        ...
        if (pathEnded != 0) {
          ...
          if (DISTANCE_TO_SPYRO(moby) < 0x2000) {
            if (func_80017908(moby->m_Rotation.z,
                              Atan2(g_Spyro.m_Position.x - moby->m_Position.x,
                                    g_Spyro.m_Position.y - moby->m_Position.y,
                                    0)) < 0x40) {
        ...
        if (func_80037F90(&props->m_Timer, 4) != 0 &&
            moby->m_Position.z - moby->m_FloorDistance - g_Spyro.m_Position.z >=
                -0x1f3) {
          int attackDistance = DISTANCE_TO_SPYRO(moby);
          pathDistance = OctDistance(
              &moby->m_Position,
              &props->m_Path->m_Nodes[props->m_Path->m_CurrentNode].m_Position);
          ...
          if (distanceOk != 0) {
            if (func_80017908(
                    Atan2(g_Spyro.m_Position.x - moby->m_Position.x,
                          g_Spyro.m_Position.y - moby->m_Position.y, 0),
                    Atan2(props->m_Path->m_Nodes[props->m_Path->m_CurrentNode]
                                  .m_Position.x -
                              moby->m_Position.x,
                          props->m_Path->m_Nodes[props->m_Path->m_CurrentNode]
                                  .m_Position.y -
      ...
      case 2:
        ...
        if (moby->m_Substate == 0) {
          ...
          angle = Atan2(props->m_Path->m_Nodes[nodeIndex[0]].m_Position.x -
                            moby->m_Position.x,
                        props->m_Path->m_Nodes[nodeIndex[0]].m_Position.y -
                            moby->m_Position.y,
                        0);
          angle2 = Atan2(props->m_Path->m_Nodes[1 - nodeIndex[0]].m_Position.x -
                             moby->m_Position.x,
                         props->m_Path->m_Nodes[1 - nodeIndex[0]].m_Position.y -
                             moby->m_Position.y,
                         0);
          ...
          angle2 = Atan2(g_Spyro.m_Position.x - moby->m_Position.x,
                         g_Spyro.m_Position.y - moby->m_Position.y, 0);
        ...
        if (D_800757F4 && moby->m_AnimationState.m_Frame == 4) {
          ...
          VecSub(
              &delta,
              &props->m_Path->m_Nodes[props->m_Path->m_CurrentNode].m_Position,
              &props->m_Path->m_Nodes[prevNode].m_Position);
      ...
      if (moby->m_Substate == 0) {
        ...
        if (keyProps->m_SparkleHandle != 0xFF) {
          ...
          VecRotateByMatrix((MATRIX *)&moby->m_RotationMatrix, &D_8006E5AC,
                            &g_Sparkles[keyProps->m_SparkleHandle].m_Position);
          VecAdd(&g_Sparkles[keyProps->m_SparkleHandle].m_Position,
                 &g_Sparkles[keyProps->m_SparkleHandle].m_Position,
                 &moby->m_Position);
...
#define HAS_BALLOONIST
    ...
    case MOBYCLASS_LOCKED_CHEST: {
      ...
      if (moby->m_Substate < 2) {
        if (props->m_Timer != 0) {
          ...
          if (props->m_Timer < 0x40) {
            moby->m_Rotation.x =
                props->m_StoredRotX + D_8006E638[props->m_Timer >> 1].x;
            moby->m_Rotation.y =
                props->m_StoredRotY + D_8006E638[props->m_Timer >> 1].y;
            moby->m_Position.z = props->m_StoredPosZ +
                                 (ABS2(D_8006E638[props->m_Timer >> 1].x) +
                                  ABS2(D_8006E638[props->m_Timer >> 1].y)) *
                                     8;
          } else {
            props->m_Timer = 0;
      ...
      case 1:
        if (g_KeyFlag == 1 &&
            ...
            ABS2(moby->m_Position.z - moby->m_FloorDistance -
                 g_Spyro.m_Position.z) < 0x200) {
          if (func_80017908(g_Spyro.m_bodyRotation.z,
                            ANGLE_TO(moby->m_Position, g_Spyro.m_Position)) <
              0x30) {
            if (func_80017908(moby->m_Rotation.z,
                              ANGLE_TO_MOBY(g_Spyro.m_Position)) < 0x30) {
              g_Spyro.m_ControlFlags = 0x80002000;
              func_8003DFA4();
              VecNull(&g_Spyro.m_HeadLookTarget);
      ...
      case 2: {
        ...
        if (props->m_Timer < 0xc0) {
          ...
          g_Spyro.m_HeadLookTarget.y =
              SINE_8((props->m_Timer * 128) / 192) >> 3;
          ...
          if (props->m_Timer < 0x40) {
            ...
            keyMoby->m_Rotation.z =
                rotTemp + ((angleDelta * (props->m_Timer - 0x80)) / 0x40);
          }
    ...
    case MOBYCLASS_METAL_CHEST: { /* MOBYCLASS_METAL_CHEST */
      ...
      if (chestProps->m_ShakeTimer != 0) {
        ...
        if (chestProps->m_ShakeTimer < 0x40) {
          /* shake animation */
          moby->m_Rotation.x = D_8006E638[chestProps->m_ShakeTimer >> 1].x +
                               chestProps->m_StoredRotX;
          moby->m_Rotation.y = D_8006E638[chestProps->m_ShakeTimer >> 1].y +
                               chestProps->m_StoredRotY;
          moby->m_Position.z =
              chestProps->m_StoredPosZ +
              (ABS2(D_8006E638[chestProps->m_ShakeTimer >> 1].x) +
               ABS2(D_8006E638[chestProps->m_ShakeTimer >> 1].y)) *
                  6;
        } else {
      ...
      if (moby->m_DamageFlags & (0x20000 | 0x80000)) {
        ...
        chestProps->m_FlameTimer =
            ApplyFlameHeatExternal(moby, chestProps->m_FlameTimer);
    ...
    case MOBYCLASS_FODDER_RAT: {
      ...
      if ((moby->m_DamageFlags & (0x10000 | 0x20000 | 0x80000)) &&
          moby->m_State != 1) {
        enemyProps->m_TargetAngle =
            func_80038178(Atan2(moby->m_Position.x - g_Spyro.m_Position.x,
                                moby->m_Position.y - g_Spyro.m_Position.y, 0),
        ...
        case 1:
          if (enemyProps->m_KnockbackSpeed >= 16) {
            ...
            func_80039688(moby, enemyProps->m_TargetAngle,
                          enemyProps->m_KnockbackSpeed, 0, 0x1F4, 5);
          }
          if (D_80075794 != 0) {
            func_800529E4(moby, 4);
    ...
    case MOBYCLASS_CRYSTAL_DRAGON: {
      ...
      if (moby->m_State == 0) {
        ...
        if (dragonProps->m_ShakeTimer > 256) {
          ...
          moby->m_Rotation.x =
              D_8006E638[(dragonProps->m_ShakeTimer - 192) >> 1].x +
              dragonProps->m_AngleStorage.x;
          moby->m_Rotation.y =
              D_8006E638[(dragonProps->m_ShakeTimer - 192) >> 1].y +
              dragonProps->m_AngleStorage.y;
          moby->m_Position.z =
              dragonProps->m_AngleStorage.z +
              (ABS2(D_8006E638[(dragonProps->m_ShakeTimer - 192) >> 1].x) +
               ABS2(D_8006E638[(dragonProps->m_ShakeTimer - 192) >> 1].y)) *
                  6;
        ...
        if (digitProps->m_Lifetime >= 1) {
          ...
          if (moby->m_Position.z > 1023) {
            ...
            if (func_8004BE4C(&moby->m_Position, 0x100, 0x100)) {
              ...
              dot = (digitProps->m_VelocityX * g_CollisionNormal.x +
      ...
      case 2: {
        ...
        case 1:
          if (ambProps->m_PathTimer-- > 0) {
            ...
            int curr, nxt;
            moby->m_Position.x =
                ambProps->m_PathPoints
                    ->m_Nodes[(ambProps->m_PathPoints->m_CurrentNode + 1) %
                              ambProps->m_PathPoints->m_NodeCount]
                    .m_Position.x;
            moby->m_Position.y =
                ambProps->m_PathPoints
                    ->m_Nodes[(ambProps->m_PathPoints->m_CurrentNode + 1) %
            ...
            ambProps->m_PathPoints->m_CurrentNode =
                (ambProps->m_PathPoints->m_CurrentNode + 1) %
                ambProps->m_PathPoints->m_NodeCount;
            curr = ambProps->m_PathPoints->m_CurrentNode;
            nxt = (curr + 1) % ambProps->m_PathPoints->m_NodeCount;
            ambProps->m_PathTimer = func_80017D7C(
                (int)&ambProps->m_PathPoints->m_Nodes[curr].m_Position,
                (int)&ambProps->m_PathPoints->m_Nodes[nxt].m_Position,
                (void *)&ambProps->m_PathVelocity.x, ambProps->m_PathSpeed);
          }
          if (moby->m_SoundChannel == 0x7F &&
              ambProps->m_PathTimer == (ambProps->m_PathTimer / 48) * 48) {
            soundTableIndex = 14;
          }
          break;
        }
...
#if defined(HAS_FROG_FODDER) || defined(HAS_RAT_FODDER)
    case MOBYCLASS_FODDER_RESPAWN: { // Fodder Respawn
      ...
      if (func_80037F90(&respProps->m_RespawnTimer, 4) != 0) {
        ...
        for (m = g_LevelMobys; m < g_DynMobys; m++) {
          if (m->m_Class == respProps->m_TargetClass && m->m_State >= 0x80 &&
...
#ifdef HAS_DRAGON
    case MOBYCLASS_DRAGON_PAD: { // Dragon pad
      switch (moby->m_State) {
      case 0: {
    ...
    case 398: {
      ...
      case 1: {
        ...
        VecAdd(&vortexPathMidpoint, &p->m_Nodes[0].m_Position,
               &p->m_Nodes[1].m_Position);
        VecShiftRight(&vortexPathMidpoint, 1);

        VecSub(&g_Spyro.unk_0x208, &vortexPathMidpoint, &g_Spyro.m_Position);
        VecAdd(&g_Spyro.unk_0x208, &g_Spyro.unk_0x208,
               &vortexProps->m_FlyInOffset);
        ...
        if (mag > 128) {
          ...
          if (moby->m_Substate != 0) {
            progressed = (vortexProps->m_Path->m_CurrentNode <
                          (vortexProps->m_Path->m_NodeCount - 1));
          } else {
            progressed = vortexProps->m_Path->m_CurrentNode;
          }
        ...
        if (func_8004BE4C(&g_Camera.m_Position, 0x300, 0x300) != 0 &&
          ...
          if (vortexProps->m_CameraPresetIndex >= 0) {
            g_Camera.m_SphericalPreset =
                &D_8006CA24[vortexProps->m_CameraPresetIndex];
          } else {
...
#ifdef HAS_FROG_FODDER
    case MOBYCLASS_FODDER_FROG: {
      Moby412Props *props;
      int angle;
      ...
      case 2:
        MoveMobyWithGravity(moby, &props->m_KnockbackTimer,
                            props->m_KnockbackAngle, 0, 0xc, 0);
    ...
    case MOBYCLASS_BALLOON: { // Balloon
      ...
      moby->m_Position.z =
          balloonProps->m_StartZ + (SINE_8(moby->m_Substate) >> 4);
      moby->m_Substate += g_DeltaTime;
      break;
    }
#endif
...
#ifdef HAS_EXTRA_LIFE_CHEST
    case MOBYCLASS_EXTRA_LIFE_CHEST: {
      Moby421Props *props;
      Moby421Props **drop;
      int shakeX;
      int shakeY;
      ...
      if (moby->m_State == 0) {
        ...
        if (props->m_Timer >= 0x1e1) {
          ...
          moby->m_Rotation.x =
              props->m_StoredRotX + D_8006E638[(props->m_Timer - 416) >> 1].x;
          moby->m_Rotation.y =
              props->m_StoredRotY + D_8006E638[(props->m_Timer - 416) >> 1].y;
          moby->m_Position.z =
              props->m_StoredPosZ +
              (ABS2(D_8006E638[(props->m_Timer - 416) >> 1].x) +
               ABS2(D_8006E638[(props->m_Timer - 416) >> 1].y)) *
                  6;
    ...
    case MOBYCLASS_LETTER_Z: { // Letters
      ...
      if (letterProps->m_Parent->m_State == 0 ||
        ...
        if (DISTANCE_TO_SPYRO(letterProps->m_Parent) < 0x400) {
          ...
          moby->m_Substate += 8;
          moby->m_Rotation.z = moby->m_Rotation.z - (oldval * 3 >> 9) +
                               (COSINE_8(moby->m_Substate) * 3 >> 9);
        } else {
          ...
          moby->m_Substate += 8;
          anglespyro =
              ANGLE_TO(g_Spyro.m_Position, letterProps->m_Parent->m_Position);
          ...
          moby->m_Rotation.z =
              anglespyro + (COSINE_8(moby->m_Substate) * 3 >> 9);
          VecMult(&letterCenteringOffset, &letterSideStep,
                  letterProps->m_Len - 1);
          VecShiftLeft(&letterSideStep, 1);
          anglespyro = 2;
          anglespyro = (letterProps->m_Len - 1) * anglespyro;
          letterCenteringOffset.z += COSINE_8(anglespyro) * 3 >> 3;
          letterWaveOriginOffset.x =
              letterForwardOffset.x * COSINE_8(anglespyro);
          letterWaveOriginOffset.y =
              letterForwardOffset.y * COSINE_8(anglespyro);
          ...
          VecSub(&letterCenteringOffset, &letterCenteringOffset,
                 &letterSideStep);
          ...
          moby->m_Position.z =
              moby->m_Position.z + (COSINE_8(anglespyro) * 3 >> 3) + 0x600;
        }
      } else {
        moby->m_Substate += 8;
        moby->m_Rotation.z =
            moby->m_FloorDistance + (COSINE_8(moby->m_Substate) * 3 >> 9);
      }
      break;
    }
    }
